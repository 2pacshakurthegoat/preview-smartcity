import { Agent, Building, Position, WorldState, SimulationEvent, Road, RoadStatus, AgentStatus, DirectorInstruction, Asset, DirectorInstructionAssetOp, WorldEffects } from '@/types/simulation';
...
export const updateWorldState = (state: WorldState): WorldState => {
  ...
  // Update assets TTL and remove expired
  const updatedAssets = (state.assets || [])
    .map(a => ({ ...a, ttl: typeof a.ttl === 'number' ? a.ttl - 1 : a.ttl }))
    .filter(a => (typeof a.ttl === 'number' ? a.ttl! > 0 : true));
  
  const nextShake = Math.max(0, (state.effects?.shake || 0) - 1);
  
  return {
    ...state,
    agents: updatedAgents,
    assets: updatedAssets,
    effects: { ...(state.effects || {}), shake: nextShake },
  };
};
...
export const applyDirectorInstructions = (
  ...
  // Apply asset ops
  let newAssets: Asset[] = [...(state.assets || [])];
  (assetsOps || []).forEach(op => {
    if (op.op === 'add') {
      newAssets.push({ id: `asset-${Date.now()}-${Math.random()}`, kind: op.kind, position: op.position, ttl: op.ttl });
    } else if (op.op === 'remove') {
      newAssets = newAssets.filter(a => !(a.kind === op.kind && a.position.x === op.position.x && a.position.y === op.position.y));
    }
  });
  ...
  if (shakeWorld) {
    effects.shake = Math.max(effects.shake || 0, 600); // ~10s at 60fps/ticks
  }
...
export const applyEventToWorld = (state: WorldState, event: SimulationEvent): WorldState => {
  ...
  if (event.type === 'accident') {
    newAssets.push({ id: `asset-${Date.now()}-${Math.random()}`, kind: 'fire', position: event.position, ttl: 1000 });
  }
  // emergencies could destroy buildings (demo effect)
  if (event.type === 'emergency') {
    newAssets.push({ id: `asset-${Date.now()}-${Math.random()}`, kind: 'destroyed_building', position: event.position, ttl: 2000 });
  }
