...
Deno.serve(async (req) => {
  ...
  try {
    const { worldState, baseUrl: clientBaseUrl, model: clientModel } = await req.json() as { worldState: WorldState, baseUrl?: string, model?: string };
    
    if (!worldState) {
      throw new Error('World state is required');
    }

    // Determine provider configuration (OpenAI-compatible)
    const DEFAULT_BASE_URL = Deno.env.get('OPENAI_BASE_URL') || 'https://api.cerebras.ai/v1';
    const baseUrl = clientBaseUrl || DEFAULT_BASE_URL;

    const DEFAULT_MODEL = Deno.env.get('OPENAI_MODEL') || 'gpt-oss-120b';
    const model = clientModel || DEFAULT_MODEL;

    // Choose API key based on base URL
    const isCerebras = baseUrl.includes('cerebras');
    const apiKey = isCerebras ? Deno.env.get('CEREBRAS_API_KEY') : Deno.env.get('OPENAI_API_KEY');

    if (!apiKey) {
      throw new Error(isCerebras ? 'CEREBRAS_API_KEY is not configured' : 'OPENAI_API_KEY is not configured');
    }
...
Analyze the situation and provide instructions for ALL agents to optimize city flow and respond to events.`;
    ...
    console.log('Calling provider for director analysis...', baseUrl, model);
